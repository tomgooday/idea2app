...
- **Open / not yet decided:** CMS approach beyond "content in DB," SEO/content-marketing plan. AU-specific billing details (GST, ABN, invoicing entity) are resolved by the Lemon Squeezy merchant-of-record decision below.
...
- **Merchant of record: Lemon Squeezy (resolves the open item in this section and in `docs/growth-playbook/01-time-leverage-and-operations.md`):** Lemon Squeezy will act as merchant of record for DIY and PRO checkout, handling GST/ABN/invoicing so it isn't the founder's job. Not yet integrated — see Section 24's known gaps. Paddle was the other option considered and is no longer under consideration.
- **Exact price points (supersedes the $99–299 / $500–1,500 ranges in Section 14/23 pricing-model entry):** **DIY = $199** one-time, **PRO = $999** one-time. Done With You's $3k–10k+ range and exact price-within-range remain open (see growth playbook Section "Open decisions").
- **DIY/PRO delivery & access mechanism (once Lemon Squeezy checkout is live):** a Lemon Squeezy purchase webhook grants access rather than Lemon Squeezy's native file-delivery — chosen because PRO needs ongoing gated access (community, dashboard, expert review) that a one-off file download can't cover, and using the same mechanism for both avoids building two delivery paths. Since there's no auth/DB yet (Build Pass 2), the leanest version that stays in Build Pass 1 scope is a **stateless, DB-free access grant**: the webhook (or a server-side check against the Lemon Squeezy API using the order ID from the checkout return URL) issues a signed, expiring token embedded in a link/cookie that unlocks gated content for that purchase — no accounts, no database row per customer. This is a stopgap like the existing constant-based content gating, not real multi-device/multi-session entitlement; a real accounts-backed system remains a Build Pass 2 item. Not yet built — see Section 24.
- **Testing without an ABN first:** stakeholder doesn't have an ABN yet and wants to validate the concept before registering one. Lemon Squeezy doesn't require an ABN/business registration to create a store - only personal ID verification and a W-8 form (name, tax classification, country of residence) for non-US individuals - so this isn't a blocker for Lemon Squeezy sandbox testing or even an initial live launch as an individual. Plan to register an ABN once real, ongoing sales activity (beyond Lemon Squeezy Test mode) makes this a genuine business rather than a test, per general ATO "carrying on an enterprise" guidance - not a fixed revenue trigger. Not formal tax advice; revisit with an accountant once live sales start.
- **Contact/support email: `support@idea2app.co`** — replaces the placeholder `hello@idea2app.co` used in Build Pass 1's pricing-page `mailto:` CTAs. Centralised as `siteConfig.supportEmail` in `src/lib/config.ts` rather than hard-coded per link.
